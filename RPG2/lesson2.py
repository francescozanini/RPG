...
# character01 = RPG.Character('custom01', 'NPC', max_hp=player.hp, max_mp=player.mp, ap=player.ap, actions=player.actions)
character01 = RPG.Character.init_given_character('Warrior', False)
epochs = 10
...
for epoc in range(0, epochs):
    for i in range(0, enemies):
        print(i)
        character02 = RPG.Character('custom02', 'NPC', max_hp=enemy_table[i][0], max_mp=enemy_table[i][1],
                                ap=enemy_table[i][2], actions=enemy_table[i][3])
        f01, f02, n_round, remain_hp, remaining_mp = fight.battle(character01, character02, save_battle=False)
